...
export default function PropertyCard({ property, className = "" }: PropertyCardProps) {
  ...
  const badgeClasses: Record<string, string> = {
    ...
    'nordic-dark': 'bg-nordic-dark/90 text-white',
    'white': 'bg-white/90 text-nordic-dark'
  };

  const badgeClass = `${badgeClasses[property.badge_color ?? 'mosque']} text-xs font-bold px-2 py-1 rounded`;
  const isRent = property.badge_color === 'mosque';
  ...
  return (
    <article className={`bg-white rounded-xl overflow-hidden shadow-card hover:shadow-soft transition-all duration-300 group cursor-pointer h-full flex flex-col ${className}`}>
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {property.is_new && (
          <div className="absolute top-3 left-3 bg-mosque text-white text-xs font-bold px-2 py-1 rounded shadow-md uppercase tracking-wider">
            New
          </div>
        )}
